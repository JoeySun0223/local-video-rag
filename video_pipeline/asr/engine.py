...
class Qwen3ASRProvider:
    ...
    def _run(
        ...
        if raw_segments is None:
            ...
            emit_progress("asr", 10, 100, "正在加载语音识别模型")
            ...
            emit_progress("asr", 15, 100, f"模型已加载，开始识别 {len(clips)} 个音频块")
            ...
            try:
                for clip_number, clip in enumerate(clips, 1):
                    ...
                    emit_progress(
                        "asr", 15 + 50 * clip_number / max(len(clips), 1), 100,
                        f"已识别音频块 {clip_number}/{len(clips)}",
                    )
            ...
            emit_progress("asr", 65, 100, f"已恢复识别检查点，共 {len(clips)} 个音频块")

        align_started = time.perf_counter()
        emit_progress("asr", 67, 100, "正在加载时间轴对齐模型")
        ...
        emit_progress("asr", 70, 100, f"开始对齐 {len(raw_segments)} 个音频块的时间轴")
        ...
        try:
            for segment_number, segment in enumerate(raw_segments, 1):
                if request.cancelled and request.cancelled():
                    raise InterruptedError("用户安全取消入库")
                if not segment["raw_transcription"].strip():
                    segment["timestamps"] = []
                    emit_progress(
                        "asr", 70 + 28 * segment_number / max(len(raw_segments), 1), 100,
                        f"已对齐音频块 {segment_number}/{len(raw_segments)}",
                    )
                    continue
    ...
    def transcribe(self, request: ASRRequest) -> ASRResult:
        ...
        emit_progress("asr", 10, 100, f"音频已切为 {len(clips)} 个识别块")
        with _MemorySampler() as memory:
            started = time.perf_counter()
...
def get_asr_provider(config: dict[str, Any], name: str | None = None):
    provider = str(name or config.get("asr", {}).get("primary", "qwen3_asr")).strip().lower()
